# From Jan Hendrik: changed lines marked #JH
...
def main():
    ...
    # print("Plug in a USB gamepad. Do it! Do it now! Press enter after you have done this.") #JH
    # wait_for_enter() #JH
    ...
    while not input_manager.quit_attempt:
        ...
        if not is_configured:
            success = True #configure_phase(screen, input_manager.buttons[button_index], input_manager) #JH
            input_manager.joystick_config = {'up': ('is_axis', 3, -1), 'down': ('is_axis', 3, 1), 'left': ('is_axis', 4, -1), 'right': ('is_axis', 4, 1), 'start': ('is_button', 7), 'A': ('is_button', 0), 'B': ('is_button', 1), 'X': ('is_button', 2), 'Y': ('is_button', 3), 'L': ('is_button', 4), 'R': ('is_button', 5)}
